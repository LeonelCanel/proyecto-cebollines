      ...
      <div class="carousel-inner">
        <div class="carousel-item active">
            <?php 
                ...
                $result = mysqli_query($mysqli, "SELECT * FROM carruselgt ORDER BY fecha_creacion ASC LIMIT 1");
                while ($row = mysqli_fetch_assoc($result)):
                    $uuid_obtenido = $row['uuid_carrusel_gt'];
                    $ruta = $row['ruta_carrusel_gt'];
                    $ruta = substr($ruta, 9);
            ?>
        ...
        <?php
            include 'Administrador/pages/conexion/conexion.php';
            $result = mysqli_query($mysqli, "SELECT * FROM carruselgt WHERE uuid_carrusel_gt <> '$uuid_obtenido'");
            while ($row = mysqli_fetch_assoc($result)):
                $ruta = $row['ruta_carrusel_gt'];
                $ruta = substr($ruta, 9);
                $nombre = $row['nombre_carrusel_gt'];
        ?>
